...
def create_poster(
    ...
    fetch_steps = 4 if THEME.get("ocean") else 3
    with tqdm(
        total=fetch_steps,
        ...
        # 4. Fetch Land polygons (only when theme has 'ocean' color defined)
        # Used for coastal cities: sets ocean as background, overlays land polygons in bg color
        land = None
        if THEME.get("ocean"):
            pbar.set_description("Downloading land cover (coastal mode)")
            land = fetch_features(
                fetch_point,
                fetch_dist,
                tags={
    ...
    # 2. Setup Plot
    print("Rendering map...")
    # In coastal mode (ocean color defined): background = ocean color, land overlaid in bg color
    ocean_color = THEME.get("ocean")
    bg_color = THEME["bg"]
    canvas_color = ocean_color if ocean_color else bg_color
    fig, ax = plt.subplots(figsize=(width, height), facecolor=canvas_color)
    ...
    # 3. Plot Layers
    # Layer 0 (coastal mode only): land polygons in bg color over ocean-colored background
    # This makes sea areas show ocean_color (canvas) while land areas show bg_color
    if ocean_color and land is not None and not land.empty:
        land_polys = land[land.geometry.type.isin(["Polygon", "MultiPolygon"])]
        if not land_polys.empty:
            try:
                land_polys = ox.projection.project_gdf(land_polys)
            except Exception:
                land_polys = land_polys.to_crs(g_proj.graph['crs'])
            land_polys.plot(ax=ax, facecolor=bg_color, edgecolor='none', zorder=0.3)
    ...
    ox.plot_graph(
        g_proj, ax=ax, bgcolor=canvas_color,
        node_size=0,
    ...
    # In ocean mode, use canvas_color (ocean blue) for fades so they don't paint green over the sea
    _default_fade = canvas_color if ocean_color else THEME["gradient_color"]
